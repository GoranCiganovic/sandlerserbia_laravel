      ...
      <div class="panel panel-default">
        ...
        <div class="panel-heading san-yell">
          ...
          <div class="text-center h3">
            <i class="fa fa-btn fa-file-text-o" aria-hidden="true"></i>Faktura Profakture br. {{  $proinvoice->proinvoice_number }}
        ...
        <div class="panel-body san-yell">
          ...
          <form method="POST" id='form' class='form_prevent_multiple_submits' action='{{ url("/invoice/update/".$contract->id."/".$payment->id."/".$invoice->id) }}'>
            <!-- Vrednost-->
            <div class="form-group{{ $errors->has('value_euro') ? ' has-error' : '' }}">
              ...
              <div class="form-control">{{ $invoice->value_euro }}</div>
              <input type="hidden" id="value_euro" name="value_euro" class="form-control" aria-describedby="value_euroHelp" value="{{ $invoice->value_euro }}" step="0.01">
            ...
            <!-- /FORM PDF ACTION -->
            <!-- FORM ISSUED ACTION -->
            <input type="hidden" id='form_issued_action' name="form_issued_action" value='{{ url("/invoice/issued/".$contract->id."/".$payment->id."/".$invoice->id) }}'>
            <!-- /FORM ISSUED ACTION -->
            <!-- FORM DELETE ACTION -->
            <input type="hidden" id='form_delete_action' name="form_delete_action" value='{{ url("/invoice/delete/".$contract->id."/".$payment->id."/".$invoice->id) }}'>
            <!-- /FORM DELETE ACTION -->
            <!-- Button  Update -->
            ...
            <div class="panel panel-default san-light">
              <button type="submit" id="issued" name="issued" class='button_prevent_multiple_submits btn btn-info btn-md btn-block'>
                   <i class="no_spinner fa fa-btn fa-check" aria-hidden="true"></i><i class='spinner fa fa-btn fa-spinner fa-spin' aria-hidden="true"></i>IZDATA <span id="issuedTypeName">FAKTURA</span>  
              </button>
            </div>
            <!--/Button  Issued -->
            <!-- PDF -->
            <div class="panel panel-default san-light">
              <button type="submit" id="pdf_preview" name="pdf_preview" class='button_prevent_multiple_submits btn btn-default btn-md btn-block'>
                   <i class="no_spinner fa fa-btn fa-file-text-o" aria-hidden="true"></i><i class='spinner fa fa-btn fa-spinner fa-spin' aria-hidden="true"></i>FAKTURA PDF 
              </button>
            </div>
            <!--/PDF -->
          </form>
